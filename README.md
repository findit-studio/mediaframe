...
A common media-stream descriptor vocabulary for media processing pipelines (pixel-format, colour, and frame metadata for video today; audio/subtitle descriptors to follow).
...
pipelines. Today it covers pixel-format, colour, and frame metadata
for video; the charter is broadened to all media-stream descriptor
vocabulary (video/audio/subtitle), with audio/subtitle types added
incrementally. Pure data types: no SIMD, no decoder, no codec, no
